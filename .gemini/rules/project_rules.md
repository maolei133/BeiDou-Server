...
## **当前协作规则补充（由项目规则与 docs 校验提炼）**

### **可复用项目画像**

- 项目定位：`BeiDou-Server-Magical` 是 Java 21 + Spring Boot 3.5.9 的冒险岛游戏服务端，配套 `gms-ui` Vue 3 / TypeScript / Vite / Pinia / Arco Design Vue 管理界面。
- 后端核心：`gms-server/` 承载游戏服、Spring Boot 管理 API、Mybatis-Flex 数据访问、GraalVM JS 脚本、WZ XML 资源、Flyway 数据库迁移、Spring Security 与 SpringDoc OpenAPI。
- 前端核心：`gms-ui/` 通过统一 Axios 客户端调用后端 API，页面层、组件层、API 层、Pinia 状态层必须职责清晰。
- 运行与日志：日志体系以 Log4j2 / Loki / Promtail / GMS-UI 查询为主，结构化日志、模块/动作枚举、日志级别边界必须与 `docs/后端/Loki日志系统集成开发文档.md` 保持一致。

### **开发前置检查**

1. 所有开发任务必须先查阅 `docs/` 下相关中文文档，至少覆盖：架构、模块职责、运行流程、对应前端/后端专题文档、开发中计划文档。
2. 涉及架构、目录结构、技术栈、异常响应、日志规范、接口协议或任务流程变化时，必须同步更新 `docs/` 中对应文档。
3. 所有任务必须进入进度中心队列；不得只在聊天中口头分配。任务应包含标题、优先级、风险、模块、验收标准、关联文档、执行人/把关人和推送目标。
4. 任务实施前需要确认当前 Git 状态；禁止误提交无关 `docs/`、`static` 或运行产物。

### **后端协作规则**

- 严守 `Controller -> Service -> Mapper -> Model` 单向依赖；Controller 不直接访问数据库，Service 不处理 HTTP 请求/响应细节。
- 管理 API 默认使用统一响应 `{code,message,data,timestamp,path}`；成功码固定 `20000`，`responseId` 仅作为兼容追踪字段保留，不替代标准字段。
- 业务失败与系统错误必须分离：`BizException`、参数校验、认证失败、权限不足、资源不存在使用 `WARN/INFO`；未捕获系统异常使用 `ERROR` 并保留完整堆栈。
- 常用业务码：参数校验 `10001`、认证失效 `20002`、权限不足 `30001`、资源不存在 `40404`、系统错误 `50000`。
- 安全组件（JWT 校验、认证入口、权限异常）不得把可预期认证/授权失败记录为 `ERROR`，应返回统一 JSON。
- 游戏脚本通过 `AbstractScriptManager` 体系管理，事件实例通过 `EventInstanceManager` 管理，WZ 数据访问必须关注一致性和性能。

### **前端协作规则**

- 所有 API 调用必须封装在 `gms-ui/src/api/`，通过统一 Axios 实例和 `gms-ui/src/api/interceptor.ts` 处理。
- 前端成功判断以后端 `code === 20000` 为准；拦截器返回 `data`，页面组件不重复解析响应外壳。
- `20002`/HTTP `401` 统一登录失效处理，`30001`/HTTP `403` 统一权限提示，`50000`/HTTP `5xx` 统一系统异常提示，其他业务失败集中警告提示。
- 页面组件只处理加载态、空态、局部 UI 状态，避免重复弹错；文件下载请求可走 `Blob` 分支，但错误路径仍需统一提示。
- `gms-ui` 代码遵循现有 Prettier/ESLint：2 空格、分号、单引号、80 列、Vue `<script>`/`<style>` 缩进、Airbnb + Vue3 + TypeScript + Prettier。

### **文档与命名规则**

- 新增或迁移文档统一放入 `/docs` 对应中文分类目录：`前端/`、`后端/`、`后端/架构/`、`后端/脚本/`、`后端/工具/`、`开发中/` 等。
- 文档文件名使用中文，描述清楚模块、功能和用途；迁移旧英文分类文档时，应保留内容要点并补充中文标题、路径和维护状态。
- 文档中若出现落后于当前仓库结构/技术栈/端口/日志规范的描述，必须在相关任务中纳入修订。

### **验证与交付规则**

- 后端变更至少验证编译/测试、接口响应结构、异常日志级别、Loki/Promtail 或日志文件输出边界。
- 前端变更至少验证 lint/build、响应拦截器行为、Token 失效/权限不足/参数错误/网络错误等场景。
- 跨端变更必须补充联调验收：后端 code/message/path/timestamp 与前端拦截器、页面提示、登录态处理一致。
