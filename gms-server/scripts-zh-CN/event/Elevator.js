...
function goingUpNow() {
    ...
    const mf = em.getChannelServer().getMapFactory();
    mf.getMap(MAP_ID.DOWN_RIDE).warpEveryone(MAP_ID.DOWN_ARRIVE,7);
    mf.getMap(MAP_ID.DOWN_ARRIVE).broadcastClock(Math.max(1, Math.floor(rideTime / 1000) + COUNTDOWN_OFFSET_SEC));
...
function goingDownNow() {
    ...
    const mf = em.getChannelServer().getMapFactory();
    mf.getMap(MAP_ID.UP_RIDE).warpEveryone(MAP_ID.UP_ARRIVE,7);
    mf.getMap(MAP_ID.UP_ARRIVE).broadcastClock(Math.max(1, Math.floor(rideTime / 1000) + COUNTDOWN_OFFSET_SEC));
    mf.getMap(MAP_ID.UP_WAIT).setReactorState();
...
function isUpNow() {
    ...
    mf.getMap(MAP_ID.DOWN_ARRIVE).warpEveryone(MAP_ID.UP_WAIT, 5);  // 在电梯门口
    goDown();
}
...
function isDownNow() {
    ...
    mf.getMap(MAP_ID.UP_ARRIVE).warpEveryone(MAP_ID.DOWN_WAIT, 5);  // 在电梯门口
    goUp();
}
