            ...
            if (targetMapId != -1) {
                if (!chr.isAlive()) {
                    ...
                    if (wheel && chr.haveItemWithId(ItemId.WHEEL_OF_FORTUNE, false)) {
                        ...
                        AuditLogger.info(LogModule.CHARACTER, LogAction.CHARACTER_REVIVE, new MapMessage().with("map", map.getId()).with("item", ItemId.WHEEL_OF_FORTUNE));
                    } else {
                        ...
                        if (executeStandardPath) {
                            ...
                            AuditLogger.info(LogModule.CHARACTER, LogAction.CHARACTER_REVIVE, new MapMessage().with("map", map.getReturnMapId()));
                        }
                    }
                } else {
                    if (chr.isGM()) {
