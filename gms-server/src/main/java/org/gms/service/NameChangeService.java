...
import static org.gms.dao.entity.table.NamechangesDOTableDef.NAMECHANGES_DO;
import static org.gms.dao.entity.table.RingsDOTableDef.RINGS_DO;
...
public class NameChangeService {
    ...
    public void applyNameChange(int characterId, String characterName) {//下线时检测是否需要更换昵称并应用
        List<NamechangesDO> namechangesDOList = namechangesMapper.selectListByQuery(QueryWrapper.create()
                .where(NAMECHANGES_DO.COMPLETION_TIME.isNull()).and(NAMECHANGES_DO.CHARACTERID.eq(characterId)));
    ...
    public List<NamechangesDO> getAllNameChanges() {
        return namechangesMapper.selectListByQuery(QueryWrapper.create().where(NAMECHANGES_DO.COMPLETION_TIME.isNull()));
    }
    ...
    public void doNameChange(NamechangesDO data) {//应用昵称更改
        ...
        ringsMapper.updateByQuery(RingsDO.builder().partnerName(data.getNewer()).build(), QueryWrapper.create().where(RINGS_DO.PARTNER_NAME.eq(data.getOlder())));
        ...
        inventoryitemsmapper.deleteByQuery(QueryWrapper.create().where(INVENTORYITEMS_DO.ITEMID.eq(ItemId.NAME_CHANGE)).and(INVENTORYITEMS_DO.CHARACTERID.eq(data.getCharacterid()).or(INVENTORYITEMS_DO.ACCOUNTID.eq(accountid))));
    ...
    public boolean registerNameChange(Character chr, String newName) {
        List<NamechangesDO> namechangesDOList = namechangesMapper.selectListByQuery(QueryWrapper.create()
                .where(NAMECHANGES_DO.CHARACTERID.eq(chr.getId())));
        // 已有改名未生效或改名未冷却
    ...
    public void cancelPendingNameChange(Character chr, boolean needFinish) {
        QueryWrapper queryWrapper = QueryWrapper.create().where(NAMECHANGES_DO.CHARACTERID.eq(chr.getId()));
        if (needFinish) queryWrapper.and(NAMECHANGES_DO.COMPLETION_TIME.isNull());
