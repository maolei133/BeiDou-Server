...
import static org.gms.dao.entity.table.MakercreatedataDOTableDef.MAKERCREATEDATA_DO;
import static org.gms.dao.entity.table.MakerreagentdataDOTableDef.MAKERREAGENTDATA_DO;
import static org.gms.dao.entity.table.MakerrecipedataDOTableDef.MAKERRECIPEDATA_DO;
...
public class ItemInformationService {
    ...
    public Map<Integer, Integer> loadCardIdData() {
        ...
        List<MonstercarddataDO> list = monstercarddataMapper.selectListByQuery(QueryWrapper.create().select(MONSTERCARDDATA_DO.CARDID, MONSTERCARDDATA_DO.MOBID));
    ...
    public Pair<String, Integer> getMakerReagentStatUpgrade(int itemId) {
        MakerreagentdataDO data = makerreagentdataMapper.selectOneByQuery(QueryWrapper.create()
                .select(MAKERREAGENTDATA_DO.STAT, MAKERREAGENTDATA_DO.VALUE)
                .where(MAKERREAGENTDATA_DO.ITEMID.eq(itemId)));
        if (data != null) {
    ...
    public List<Integer> getMakerCrystalFromLeftover(int leftoverId) {
        return dropDataMapper.selectListByQueryAs(QueryWrapper.create()
                ...
                .where(DROP_DATA_DO.ITEMID.eq(leftoverId))
                .orderBy(DROP_DATA_DO.DROPPERID.asc()), Integer.class);
    }

    public MakercreatedataDO getMakerItemEntry(int toCreate) {
        return makercreatedataMapper.selectOneByQuery(QueryWrapper.create()
                .select(MAKERCREATEDATA_DO.REQ_LEVEL, MAKERCREATEDATA_DO.REQ_MAKER_LEVEL, MAKERCREATEDATA_DO.REQ_MESO, MAKERCREATEDATA_DO.QUANTITY)
                .where(MAKERCREATEDATA_DO.ITEMID.eq(toCreate)));
    }
    ...
    public List<Pair<Integer, Integer>> getMakerItemRecipe(int toCreate) {
        List<MakerrecipedataDO> list = makerrecipedataMapper.selectListByQuery(QueryWrapper.create()
                .select(MAKERRECIPEDATA_DO.REQ_ITEM, MAKERRECIPEDATA_DO.COUNT)
                .where(MAKERRECIPEDATA_DO.ITEMID.eq(toCreate)));
    ...
    public List<Pair<Integer, Integer>> getMakerDisassembledItems(int itemId) {
        List<MakerrecipedataDO> list = makerrecipedataMapper.selectListByQuery(QueryWrapper.create()
                .select(MAKERRECIPEDATA_DO.REQ_ITEM, MAKERRECIPEDATA_DO.COUNT)
                .where(MAKERRECIPEDATA_DO.ITEMID.eq(itemId))
                .and(MAKERRECIPEDATA_DO.REQ_ITEM.ge(4260000))
                .and(MAKERRECIPEDATA_DO.REQ_ITEM.lt(4270000)));
        List<Pair<Integer, Integer>> result = new LinkedList<>();
        for (MakerrecipedataDO data : list) {
    ...
    public Integer getMakerDisassembledFee(int itemId) {
        MakercreatedataDO data = makercreatedataMapper.selectOneByQuery(QueryWrapper.create()
                .select(MAKERCREATEDATA_DO.REQ_MESO)
                .where(MAKERCREATEDATA_DO.ITEMID.eq(itemId)));
        if (data != null) {
